...
function parametersCard(ctx) {
  ...
  return h("section", { class: "card", "aria-labelledby": "h-params" },
    ...
    h("dl", { class: "dl params" },
      ...
      row("Come si uniscono Jev e prezzo", st.blend_method === "linear" ? "media lineare" : "media in log-odds", "BLEND_METHOD"),
      row("Calibrazione di Jev (a, b)", `${fmt.num3(st.jev_calib_a ?? 0)} · ${fmt.num3(st.jev_calib_b ?? 1)}`, "JEV_CALIB_A / JEV_CALIB_B"),
      row("Chiamate a Jev per previsione", String(st.jev_samples ?? 1), "JEV_SAMPLES"),
