...
| GET | `/portfolio/shadow` | Scommesse bloccate dai filtri, seguite senza soldi: per filtro (`summary`) e una per una (`bets`) |
| POST | `/portfolio/orders/{id}/cancel` | Annulla un ordine limite in attesa (admin) |
| GET | `/portfolio/export` | Tutto il portafoglio adesso: `format=xlsx` (riepilogo, scommesse, ordini, scommesse ombra, capitale, esclusioni, colonne) o `format=csv` (scommesse); `lang=it/en` per le descrizioni |
